...
public class UserController {
    ...
    @PostMapping
    public ReturnUserDto createUser(@RequestBody CreateUserDto dtoUser) {
        log.info("Пришёл запрос на создание пользователя с email: {}.", dtoUser.getEmail());
        return userService.createUser(dtoUser);
    }

    @PatchMapping("/{id}")
    public ReturnUserDto updateUser(@PathVariable Long id, @RequestBody UpdateUserDto dtoUser) {
