...
public class BookingController {
    ...
    @PostMapping
    public ReturnBookingDto createBooking(@RequestBody BookingDto dtoBooking,
    ...
    @GetMapping
    public List<ReturnBookingDto> getBookings(@RequestParam(name = "state", defaultValue = "ALL") String state,
                                                 @RequestHeader("X-Sharer-User-Id") Long bookerId) {
        log.info("Пришел запрос на получения бронирований для пользователя с id = {}", bookerId);
        return bookingService.getBookings(state, bookerId);
    }
